...
def get_all_attendance_by_userid(user_id):
    attendances = sql_select_all_by_col('SELECT attendances.id, attendances.user_id, attendances.event_id FROM attendances JOIN users ON users.id = attendances.user_id JOIN events ON events.id = attendances.event_id WHERE users.id = %s',[user_id])
    all_attendances = []
