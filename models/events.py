...
def get_all_events(user_email, user_id):
    events =  sql_select_all_by_col("SELECT id, name, type, description, location, date, start_time, end_time, email_list, user_id FROM events WHERE  (type = 'Public' OR %s = ANY(email_list) OR user_id =%s) ORDER BY date ASC",[user_email, user_id])
    all_events = []
    for event in events:
...
# def get_all_past_events(user_email, user_id):
